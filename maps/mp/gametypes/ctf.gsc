...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	wait delay * level.ec_fpsMultiplier;	// ?? Also required for Callback_PlayerKilled to complete before respawn/killcam can execute
...
autoReturn()
{
	...
	wait 120 * level.ec_fpsMultiplier;
...
menuWeapon(response)
{
	...
	else
	{
		...
		secondaryname = maps\mp\gametypes\_weapons::getWeaponName(self.pers["secondaryweapon"]);
		...
		if (maps\mp\gametypes\_weapons::useAn(self.pers["secondaryweapon"]))
			self iprintln(&"MP_YOU_WILL_RESPAWN_WITH_AN", secondaryname);
		else
			self iprintln(&"MP_YOU_WILL_RESPAWN_WITH_A", secondaryname);
	}
...
respawn_timer(delay)
{
	...
	if(level.respawndelay > 0)
	{
		...
		wait delay * level.ec_fpsMultiplier;
		self thread updateTimer();

		wait level.respawndelay * level.ec_fpsMultiplier;
