...
/** Sh0k'
Many modifications to this script that involve using the cvar ec_fpsMultiplier are adapted from PAM v2.04 in order
to accomodate for the increased tickrate the server is running.
 */
...
Callback_StartGameType()
{
	...
	//Sh0k' >>
	if (!isdefined(game["pregame"]))	
		game["pregame"] = true;

	if (!isdefined(game["halftimeStratTime"]))
	...
	//Sh0k' >>
	if (getCvar("ec_bombtimer") == "")
		setCvar("ec_bombTimer", 60);
	level.bombtimer = getcvarint("ec_bombtimer");
	//Sh0k' <<
	...
	///// Added for AWE ///////
	if(getCvar("scr_sd_plantscore") == "")
		setCvar("scr_sd_plantscore", "0");
	level.plantscore = getCvarInt("scr_sd_plantscore");

	if(getCvar("scr_sd_defusescore") == "")
		setCvar("scr_sd_defusescore", "0");
	level.defusescore = getCvarInt("scr_sd_defusescore");
	///////////////////////////
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	wait level.ec_fpsMultiplier * delay;	// ?? Also required for Callback_PlayerKilled to complete before killcam can execute
...
spawnIntermission()
{
	...
	//Sh0k' >>
	if (level.ec_devMode)
		iprintln("^7 GAME IS IN INTERMISSION");
	//Sh0k' <<
...
startRound()
{
	...
	//Don't start the round until pregame has ended
	while (game["pregame"] == true)
		wait 0.1;
	
	//Determine how long we're going to sit in stratTime
	if (game["halftimeStratTime"] == true)
	...
	if(game["matchstarted"])
	{
		...
		level.clock setTimer(level.roundlength * 60); //this here
		...
		if((level.roundlength * 60) > level.graceperiod)
		{
			...
			timer = (level.roundlength * 60) - level.graceperiod; //Sh0k' +
			//wait((level.roundlength * 60) - level.graceperiod); //Sh0k' -
		}
		else
			timer = level.roundlength * 60; //Sh0k' +
			//wait(level.roundlength * 60); //Sh0k' -
	}
	else
	{
		level.clock.color = (1, 1, 1);
	...
	for(;;)
	{
		...
		wait level.ec_fpsMultiplier * level.check_time;
		if(level.roundended)
			return;
	}
	//Sh0k' <<
...
{	
	...
	wait level.ec_fpsMultiplier * 10;
	exitLevel(false);
}
...
{	
	// Sh0k' >>
	if (level.ec_devMode == 1){
		iprintln("Checking round limit: ", level.roundlimit);
		iprintln("dvar set to: ", level.ec_sdHalftime);
		iprintln("game[roundsplayed]: ", game["roundsplayed"]);
	}
	// Sh0k' <<
	...
	//Sh0k' >>
	if (game["roundsplayed"] == level.ec_sdHalftime){
		//Exec halftime switch
		ECmod\_sdScripts::halftime();
		return;
	}
	//Sh0k' <<
	...
	for(;;)
	{
		...
		if(isPlayer(other) && (other.pers["team"] == team) && other isOnGround())
		{
			while(other istouching(self) && isAlive(other) && other useButtonPressed())
			{
				...
				{
					self.progresstime += level.frame; //Sh0k' FROM PAM v2.04
					wait level.frame; //Sh0k' FROM PAM v2.04
				}
...
bomb_countdown()
{
	...
	if (level.ec_devMode > 0)
		iprintln("^3BOMB PLANTED WAITING FOR: ",level.bombtimer * level.ec_fpsMultiplier);

	wait level.bombtimer * level.ec_fpsMultiplier;
	// Sh0k' <<
...
{	
	//Sh0k' >>
	timeleft = (level.ec_bombTimer * level.ec_fpsMultiplier);

	if (level.ec_devMode > 0){ //debug prints
		iprintln("level.bombtimer: ", level.bombtimer);
		iprintln("getTime(): ", getTime());
		iprintln("level.bombtimerstart ",level.bombtimerstart);
		iprintln("timeleft: ", timeleft);
		iprintln("level.ec_bombTimer: ", level.ec_bombTimer);
		iprintln("^7fps_multiplier: ", level.ec_fpsMultiplier);
	}
	...
	if(timeleft > 0)
	{
		...
		self.bombtimer setClock(level.bombtimer, level.bombtimer, "hudStopwatch", 48, 48);
	}
	//Sh0k' <<
}
